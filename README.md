...
P7 plugin `0.7.7` is deployed to `chris-avatar` with hardened Session/Task
ownership, pre-Handoff Context boundaries, persisted Segment anchors, and
fail-closed Emergency archive recovery. Isolated integration, real Hermes
Host/install, restart, 10-rotation, and rollback tests are complete. The first
Handoff/Emergency under a real developer
